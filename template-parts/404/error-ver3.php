<section class="error-page-wrapper ver3">
    <?php
    $image = get_field('404_image','options');
    $size = 'full';
    ?>
    <?php echo wp_get_attachment_image( $image['id'], $size ); ?>
    <div class="error-page-content-wrap primary-light-color-bg">
        <?php if (get_field('404_title','options')): ?>
            <h1 class="hero-title error third-color">
                <?php the_field('404_title','options'); ?>
            </h1>
        <?php endif; ?>
        <?php if (get_field('404_subtitle','options')): ?>
            <span class="main-section-subtitle error third-color">
                <?php the_field('404_subtitle','options'); ?>
            </span>
        <?php endif; ?>
        <?php if (get_field('404_content','options')): ?>
            <span class="main-section-text error third-color margin-bottom-big">
        ...
        <?php if (get_field('404_button_link','options') && get_field('404_button_text','options')): ?>
            ...
            <div class="button-wrapper">
                <a class="border-btn-wrap third-color hover <?php echo $btnStyle; ?>" href="<?php the_field('404_button_link','options') ?>">
